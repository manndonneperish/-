...
enum Simple { // 对应简谱，X、L、M、H 分别指超低音、低音、中音、高音
...
{
	HMIDIOUT handle;
	midiOutOpen(&handle, 0, 0, 0, CALLBACK_NULL); // 打开 midi 通道
	int volume[]{ 68, 85, 127, 60, 90 }; // 各音轨的音量，最高音量为 127，即 0x7f
	float bpm = 240;
	...
	float cycle = time_of_whole / 32; // 以 32 分音符为循环的最小单位
	int rhythm = 0;
